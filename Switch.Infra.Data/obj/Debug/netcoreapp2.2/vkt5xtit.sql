...
CREATE TABLE `Identificação` (
    ...
    CONSTRAINT `PK_Identificação` PRIMARY KEY (`Id`),
    CONSTRAINT `FK_Identificação_Usuarios_UsuarioId` FOREIGN KEY (`UsuarioId`) REFERENCES `Usuarios` (`Id`) ON DELETE CASCADE
);

CREATE UNIQUE INDEX `IX_Identificação_UsuarioId` ON `Identificação` (`UsuarioId`);

INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20190813031848_AddIdentificacao', '2.2.6-servicing-10079');
